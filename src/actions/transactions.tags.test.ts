...
describe("updateTransactionAction tags", () => {
  it("replaces tags with set when tags are provided", async () => {
    vi.mocked(prisma.transaction.findFirst).mockResolvedValue({ id: "x1", householdId: "h1" } as never);
...
describe("bulkAddTagAction", () => {
  ...
  it("connects the tag on owned rows that do not already have it", async () => {
    vi.mocked(prisma.tag.findFirst).mockResolvedValue({ id: "t1", householdId: "h1" } as never);
    ...
    expect(prisma.transaction.findMany).toHaveBeenCalledWith({
      where: { householdId: "h1", id: { in: ["x1", "x2", "not-mine"] }, NOT: { tags: { some: { id: "t1" } } } },
      select: { id: true },
    });
...
describe("bulkRemoveTagAction", () => {
  it("disconnects the tag from owned rows that have it", async () => {
    ...
    expect(res).toEqual({ ok: true });
    expect(prisma.transaction.findMany).toHaveBeenCalledWith({
      where: { householdId: "h1", id: { in: ["x1"] }, tags: { some: { id: "t1" } } },
      select: { id: true },
    });
    expect(prisma.transaction.update).toHaveBeenCalledWith({
      where: { id: "x1" },
