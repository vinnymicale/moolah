...
// boot the in-process backup and notification schedulers on a long-lived server
// (self-hosted Docker / `npm start` / `npm run dev`).
//
// Guarded to the Node.js runtime so it never runs in the edge runtime, and
// skipped in demo mode (no real backups or notifications there). Both schedulers
// are idempotent, so a repeated register() won't stack duplicate timers.
