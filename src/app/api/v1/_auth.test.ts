...
describe("requireApiHousehold", () => {
  ...
  it("returns the householdId for a valid token", async () => {
    authn.mockResolvedValue({ householdId: "h1" });
    const out = await requireApiHousehold(req({ authorization: "Bearer good" }));
    expect(out).toEqual({ ok: true, householdId: "h1" });
  });

  it("401s with a WWW-Authenticate header when the token is invalid", async () => {
    authn.mockResolvedValue(null);
    const out = await requireApiHousehold(req({ authorization: "Bearer bad" }));
  ...
  it("rate-limits anon requests by the first x-forwarded-for hop", async () => {
    authn.mockResolvedValue(null);
    await requireApiHousehold(req({ "x-forwarded-for": "203.0.113.7, 10.0.0.1" }));
