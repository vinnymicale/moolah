...
// only allowed when the requester's household is the sole one in the database
// (the self-host case); otherwise it would expose other households' data.
export async function GET() {
  ...
  const otherHouseholds = await prisma.household.count({ where: { id: { not: user.householdId } } });
  if (otherHouseholds > 0) {
    return NextResponse.json(
      { error: "Backup download is only available on single-household installations. Use the db:backup CLI script on the server instead." },
      { status: 403 },
    );
  }
