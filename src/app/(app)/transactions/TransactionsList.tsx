...
export function TransactionsList({
  ...
  // FilterSidebar renders straight from the props and every change navigates.
    ...
    <div>
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-start">
        <FilterSidebar
          ...
          dateRange={
            <div className="space-y-1.5">
              <select className="input h-8 w-full text-xs" value={range} onChange={(e) => changeRange(e.target.value)} aria-label="Time range">
              ...
              {range === "month" && (
                <div className="flex items-center gap-1">
                  <Link href={urlWith({ m: prevMonthISO.slice(0, 7) })} className="btn-ghost h-8 w-8 shrink-0 p-0!" aria-label="Previous month">
                    <ChevronLeft size={16} />
                  </Link>
                  <span className="flex-1 truncate text-center text-xs font-medium">
                    {monthLabel(new Date(`${monthISO}T00:00:00Z`))}
                  </span>
                  <Link href={urlWith({ m: nextMonthISO.slice(0, 7) })} className="btn-ghost h-8 w-8 shrink-0 p-0!" aria-label="Next month">
                    <ChevronRight size={16} />
                  </Link>
                </div>
              )}
              {range !== "month" && range !== "custom" && (
                <p className="px-1 text-xs text-muted">{rangeLabel}</p>
              )}
              {range === "custom" && (
        ...
        <div className="mb-4 flex flex-wrap items-center gap-2">
          ...
          {/* Clearing lives in the sidebar header; this is only the save half. */}
          {hasActiveFilters && (
